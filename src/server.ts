...
import type { NativeCommand } from '@lazyoft/jev-browser';
...
export function createExplorerServer(explorer: BrowserExplorer) {
  const server = new McpServer({ name: 'jev-explorer', version: '0.4.0' });
  ...
  add('jev_explore', 'Delegate a complete browser exploration objective to Jev. Supply questions whose answers are unknown; answers include verbatim source evidence. Returns a compact handoff, not the full DOM. The browser stays open for inspection and continuation. ready_for_review is evidence for the caller to review, not certified business correctness. Use only within the user-authorized scope. Jev classifies action effects; business mutations also require explicit commit permission. This is not a read-only security boundary.', z.object({
    ...
    maxTokens: z.number().int().min(1000).max(3000000).default(800000), timeoutMs: z.number().int().min(1000).max(180000).default(180000),
  ...
  add('jev_act', 'Take one explicit native browser action in the retained session, using a current ref from jev_inspect. For typing provide the text; passwords are not returned in the handoff. Clicks, key presses and selection changes receive a Jev effect check against the objective and commit permission. Then continue with jev_continue. This tool does not generate selectors or execute arbitrary JavaScript.', z.object({ sessionId: id, action: native }), ({ sessionId, action }, signal) => explorer.act(sessionId, action as NativeCommand, { signal }));
