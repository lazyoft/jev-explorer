...
import { z } from 'zod';
import { compactReport, short } from './compact.mjs';

const flatten = (value, path = '') => Object.entries(value ?? {}).flatMap(([key, item]) => {
...
const actionText = action => [action.kind, action.target?.role, action.target?.name, action.option?.label, action.key, action.direction].filter(Boolean).join(' ');
const errorInfo = error => ({ code: error?.code ?? 'OPERATION_FAILED', message: short(error?.message ?? 'Operation failed', 500) });
...
export class BrowserExplorer {
  constructor({ root = resolve('runs'), launch = options => JevBrowser.launch(options), engineFactory = () => new JevDecisionEngine({ model: process.env.JEV_MODEL ?? 'jev-1.13.0' }), maxSessions = 3, idleMs = 1800000 } = {}) {
    this.root = root;
    this.launch = launch;
  ...
  async persist(session) {
    const data = this.scrub(session, {
      ...
      inputProgress: session.applied, artifacts: session.artifacts,
    });
  ...
  wrapEngine(session, provider) {
    return { decide: async (request, options = {}) => {
      ...
      run.calls++;
      const page = request.state?.page;
      const fingerprint = page ? hash({ url: page.url, elements: page.elements?.map(({ role, name, filled, checked, disabled }) => ({ role, name, filled, checked, disabled })), texts: page.texts?.map(({ role, text }) => ({ role, text })) }) : undefined;
      if (fingerprint) {
      ...
      const augmented = this.scrub(session, {
        ...request,
        state: { observed: typeof request.state === 'object' ? undefined : request.state, ...request.state,
          explorationMemory: {
      ...
      let result;
      try { result = await provider.decide(augmented, options); }
      catch (error) { run.failedCalls++; await this.event(session, { kind: 'decision-failed', error: errorInfo(error) }); throw error; }
      ...
      if (fingerprint && selected && !selected.startsWith('__')) {
        const candidate = request.questions.action.criteria[selected];
        const key = fingerprint + ':' + hash(typeof candidate === 'object' ? { action: actionText(candidate), valueKey: candidate.valueKey } : candidate);
        const count = (run.repetitions.get(key) ?? 0) + 1;
        run.repetitions.set(key, count);
        run.lastAction = typeof candidate === 'object' ? actionText(candidate) : String(candidate);
  ...
  async open({ url, headed = false, redactions = [] }, { page, signal } = {}) {
    ...
    const session = { id, dir, core: null, touched: Date.now(), busy: false, closed: false, status: 'open', reason: 'session_created', objective: '', values: {}, questions: [], facts: [], history: [], notes: [], needs: [], applied: {}, redactions, limitations: [], allowCommit: false,
    ...
    const provider = { decide: (...args) => (session.provider ??= this.engineFactory(session)).decide(...args) };
    const options = { engine: this.wrapEngine(session, provider), headless: !headed, contextOptions: { viewport: { width: 1600, height: 1100 } }, timeoutMs: 60000,
    ...
    } catch (error) { await session.core?.close(); this.sessions.delete(id); throw error; }
    finally { this.opening--; }
  }

  async capture(session) {
    for (let attempt = 0; attempt < 3; attempt++) {
      try { session.view = await session.core.snapshot({ timeoutMs: 5000 }); break; }
      catch (error) {
    ...
    for (const frame of session.core.page.frames()) {
      const invalid = await frame.locator('[aria-invalid="true"]').evaluateAll(elements => elements.filter(element => element.getBoundingClientRect().width > 0 && element.getBoundingClientRect().height > 0).slice(0, 20).map(element => {
        const textFor = attribute => (element.getAttribute(attribute) ?? '').split(/\s+/).map(id => element.ownerDocument.getElementById(id)?.textContent ?? '').join(' ').trim();
  ...
  async remainingValues(session) {
    ...
    const copy = (value, path = '') => Object.fromEntries(Object.entries(value).flatMap(([key, item]) => {
      const next = path + '/' + key.replaceAll('~', '~0').replaceAll('/', '~1');
  ...
  async continue(id, { note = '', values = {}, facts = [], effectResolution } = {}, { signal } = {}) {
    return this.exclusive(id, async session => {
      ...
      const merge = (left, right) => {
        const output = { ...left };
        for (const [key, value] of Object.entries(right)) Object.defineProperty(output, key, { value: value && typeof value === 'object' && !Array.isArray(value) ? merge(Object.hasOwn(left, key) ? left[key] : {}, value) : value, enumerable: true, writable: true, configurable: true });
        return output;
      };
  ...
  async execute(session, signal) {
    ...
    session.status = 'running'; session.reason = 'exploring'; session.needs = [];
    ...
    try {
      ...
      } else if (result?.status !== 'complete' && result?.inputs?.some(input => input.applied) && requiredMissing.length) {
        session.status = 'needs_input';
        session.reason = 'missing_input_observed';
      }
      if (session.questions.length && result?.reason !== 'effect-unknown') {
      ...
      session.status = error.code === 'BUDGET_EXHAUSTED' ? 'budget_exhausted' : 'needs_review';
      ...
      await this.capture(session).catch(error => session.limitations.push(`Final observation unavailable: ${error.code ?? 'error'}`));
  ...
  async inspect(id, { targets = false, screenshot = false, offset = 0 } = {}) {
    return this.exclusive(id, async session => {
      await this.capture(session);
  ...
  async act(id, command, { signal } = {}) {
    return this.exclusive(id, async session => {
      if (command.ref && !session.view?.elements.some(element => element.id === command.ref)) throw new BrowserError('STALE_TARGET', 'Inspect targets and use a current observed ref.');
      const target = session.view?.elements.find(element => element.id === command.ref);
      ...
      if (target?.inputType === 'password' && typeof command.text === 'string') session.redactions.push(command.text);
