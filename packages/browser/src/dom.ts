...
function pointerPosition(el: Element): { x: number; y: number } | null | undefined {
  ...
  for (const rect of el.getClientRects()) {
    ...
    inViewport = true;
    for (const [dx, dy] of [[.5,.5],[.1,.1],[.9,.1],[.1,.9],[.9,.9]] as const) {
      const x = left + (right-left)*dx, y = top + (bottom-top)*dy;
      ...
      for (let node: Element | null = hit; node;) {
        if (node === el) return { x: x-box.left-el.clientLeft, y: y-box.top-el.clientTop };
        const root = node.getRootNode();
        node = node.parentElement ?? (root instanceof ShadowRoot ? root.host : null);
      }
    }
  }
  return inViewport ? null : undefined;
}
...
export function describe(el: Element, point = pointerPosition(el)) {
  ...
  return { info, signature, connected: el.isConnected, visible: visible(el), pointerPosition: point };
}
