...
export class JevBrowser {
  ...
  private async executeCaptured(plan: ActionPlan, captured: Captured, values: Record<string,string>, operation: Operation, started: ()=>void=()=>{}): Promise<ActResult> {
    ...
    if(ref && ['click','check','uncheck','hover'].includes(action.kind)){
      if(ref.pointerBlocked)throw new BrowserError('ACTION_UNAVAILABLE','The observed control is covered; no action was dispatched.');
      try{await ref.handle.click({trial:true,position:ref.pointerPosition,timeout:Math.min(1000,this.remaining(operation))});}
      catch{throw new BrowserError('ACTION_UNAVAILABLE','The control did not become actionable within the short preflight; no click was dispatched.');}
      operation.signal.throwIfAborted();
      await verifyTarget(ref);
      if(ref.pointerBlocked)throw new BrowserError('ACTION_UNAVAILABLE','The control became covered during preflight; no action was dispatched.');
    }
    started();
    try{
      const outcome=action.kind==='dialog'
        ...
        : await this.nativeBrowser.executeResolved(parsed,ref!.handle,op(),ref!.pointerPosition);
