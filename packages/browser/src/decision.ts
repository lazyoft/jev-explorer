// Modified by lazyoft: request preparation, conservative token checks and safe context-limit errors.
...
export interface DecisionEngine {
  prepare?(request: DecisionRequest): DecisionRequest;
  ...
  async decide(request: DecisionRequest, options: { signal?: AbortSignal; maxRetries?: number } = {}): Promise<DecisionResult> {
    ...
    try {
      ...
      const status = typeof error === 'object' && error !== null && 'status' in error && typeof error.status === 'number'
        ? ` (HTTP ${error.status})` : '';
      if (error instanceof Error && /\bmax_tokens_exceeded\b/.test(error.message)) throw new BrowserError('PROVIDER_CONTEXT_LIMIT', `Jev rejected the request${status}: max_tokens_exceeded despite the conservative estimate. Narrow the observed scope; no browser action was retried.`);
