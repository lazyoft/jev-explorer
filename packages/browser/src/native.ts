// Modified by lazyoft: bound individual element actions and use verified click positions.
...
export class NativeBrowser extends BrowserEvents {
  async executeResolved(c: ParsedNativeCommand, target: Locator | ElementHandle<Element>, op: OperationContext, position?: {x:number;y:number}): Promise<Record<string, unknown>> {
    const time = { timeout: Math.min(op.timeoutMs, 2000), signal: op.signal }; op.signal.throwIfAborted();
    return this.action(async () => {
      switch (c.command) {
        case 'click': await target.click({ ...time, position, button: c.button, clickCount: c.doubleClick ? 2 : 1, modifiers: c.modifiers }); break;
