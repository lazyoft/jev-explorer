...
export interface ElementRef { frame: Frame; handle: ElementHandle<Element>; signature: string; info: ElementInfo; pointerPosition?: {x:number;y:number}; pointerBlocked?:boolean }
...
export async function verifyTarget(ref: ElementRef): Promise<void> {
  ...
  ref.pointerPosition = current.pointerPosition ?? undefined;
  ref.pointerBlocked = current.pointerPosition === null;
