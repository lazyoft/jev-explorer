// Modified by lazyoft: exercise native cancellation separately from covered-control discovery.
...
test('cancelling an explicitly requested native blocked click prevents a later click when the cover disappears',async t=>{
  const abort=new AbortController();
  const {core,page}=await fixture(t,'<button onclick="document.body.dataset.saved=1">Save</button><div id="cover" style="position:fixed;inset:0;z-index:99"></div>',{timeoutMs:2000});
  await page.evaluate(()=>setTimeout(()=>document.getElementById('cover')?.remove(),400));
  const timer=setTimeout(()=>abort.abort(),75);t.after(()=>clearTimeout(timer));
  await assert.rejects(core.native({command:'click',target:'button'},{signal:abort.signal}));
